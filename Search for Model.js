...
const get = async (url) =>
  new Promise((resolve, reject) => {
    GM_xmlhttpRequest({
      url,
      onload: (data) => {
        if ([404, 500].includes(data.status)) reject();
        resolve(data);
      },
    });
  });
  ...
  for (const [group, sites] of Object.entries(data)) {
    for (const url of sites) {
      if (group === 'Brazzers') {
        ...
        const data = await get(`${url.replace('#', query[0])}`);
        const doc = new DOMParser().parseFromString(data.response, 'text/html');

        const links = [
          ...doc.querySelectorAll(`figcaption > a[href$="${query}"]`),
        ];
        ...
        try {
          await get(`${url.replace('#', query)}`);
          urls.push(url.replace('#', query));
        } catch (err) {}
      } else if (group === 'Porn Pros') {
        const data = await get(`${url.replace('#', query)}`);
        ...
        if (url.includes('bangbros.com')) {
          ...
          const links = [
            ...doc.querySelectorAll(`.echThumb > a[href$="${query}"]`),
          ];
          ...
          for (const variation of [query, query.replace('-', '')]) {
            try {
              const data = await get(`${url.replace('#', variation)}`);
              const doc = new DOMParser().parseFromString(
                data.response,
                'text/html'
              );
